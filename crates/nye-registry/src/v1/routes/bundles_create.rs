...
use crate::database::{Package, PackageVersion, PackageVersionBundle};
use crate::deferred::DeferredGroup;
use crate::state::State;
use crate::storage::Storage;
use crate::time;
use crate::v1::errors::{Error, ResultOrHttpError};
use crate::v1::extractors::auth::Auth;
...
pub async fn handle(state: State, auth: Auth, mut multipart: Multipart) -> Result<(), Error> {
    ...
    while let Some(mut field) = multipart.next_field().await.or_into_422()? {
        ...
        let analysis = nye_packages::analyze(&filename).await.or_http_422()?;

        if !analysis.manifest.package.target.is_supported() {
            return Err(Error::http_422());
        }

        if let Some(package_meta) = &mut package_meta {
            if package_meta.name != analysis.manifest.package.name {
                return Err(Error::http_422());
            }

            if package_meta.version != analysis.manifest.package.version {
                return Err(Error::http_422());
            }

            if package_meta
                .targets
                .contains(&analysis.manifest.package.target)
            {
                return Err(Error::http_422());
            }
            ...
            package_meta
                .targets
                .insert(analysis.manifest.package.target);
        } else {
            let package = Package::filter_by_name(&analysis.manifest.package.name)
                .first()
                .exec(&mut db)
                .await
                .or_http_500()?;
            ...
            if let Some(package) = package {
                ...
                let version = PackageVersion::filter_by_package_id_and_number(
                    &package.id,
                    analysis.manifest.package.version.to_string(),
                )
                .first()
                .exec(&mut db)
                .await
                ...
                toasty::create!(Package {
                    id: nanoid::nanoid!(),
                    name: &analysis.manifest.package.name,
            ...
            package_meta = Some(PackageMeta {
                name: analysis.manifest.package.name.clone(),
                version: analysis.manifest.package.version.clone(),
                targets: HashSet::from([analysis.manifest.package.target]),
            });
        }

        package_bundles.insert(analysis.manifest.package.target, filename);
    }
