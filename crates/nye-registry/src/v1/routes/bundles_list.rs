...
use nye_schemas::semver::SemverQuery;
use nye_schemas::targets::Target;
use nye_validation::Validate;
use serde::{Deserialize, Serialize};

use crate::database::{Package, PackageVersionBundle};
use crate::state::State;
use crate::v1::errors::{Error, OptionOrHttpError, ResultOrHttpError};
use crate::v1::schemas::Page;
...
pub async fn handle(state: State, params: Query<BundleQueryParams>) -> Result<Page<()>, Error> {
    ...
    let query = Package::filter(expr.or_http_500()?)
        ...
        .include(
            Package::fields().versions().bundles().filter(
                PackageVersionBundle::fields()
                    .target()
                    .eq(params.target.to_string()),
            ),
        )
        .exec(&mut db)
        .await
